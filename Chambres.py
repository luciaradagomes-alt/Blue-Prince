    ...
    def interaction_with_services(self) :
        ...
        while True:
            ...
            if response :
                ...
                if response == '1':
                    message.append(f"\nAvant : {self.inventory.coins} pièces et {self.inventory.gems} gemmes")
                    service_name, action = services_list[0]
                    if self.inventory.coins < action["cost"] :
                        message.append(f"Pas assez d'argent :(")
                        return
                    self.inventory.coins -= action["cost"]
                    
                    new_gems = self.inventory.coins
                    new_coins = self.inventory.gems
                    self.inventory.coins = new_coins
                    self.inventory.gems = new_gems

                    message.append("\nService complété ! Toutes vos gemmes ont été échangées par toutes vos pièces !")
                    message.append(f"\nAprès : {self.inventory.coins} pièces et {self.inventory.gems} gemmes")
                elif response == '2' :
                    message.append(f"\nAvant : {self.inventory.keys} clés et {self.inventory.gems} gemmes")
                    ...
                    new_gems = self.inventory.keys
                    new_keys = self.inventory.gems
                    self.inventory.keys = new_keys
                    self.inventory.gems = new_gems

                    message.append("\nService complété ! Toutes vos clés ont été échangées par toutes vos gemmes !")
                    message.append(f"\nAprès : {self.inventory.keys} clés et {self.inventory.gems} gemmes")
                else:
                    message.append(f"\nAvant : {self.inventory.keys} clés et {self.inventory.coins} pièces")
                    ...
                    new_coins = self.inventory.keys
                    new_keys = self.inventory.coins
                    self.inventory.keys = new_keys
                    self.inventory.coins = new_coins

                    message.append("\nService complété ! Toutes vos clés ont été échangées par toutes vos pièces !")
                    message.append(f"\nAprès : {self.inventory.keys} clés et {self.inventory.coins} pièces")
...
class Orange(Room):
    ...
    def items(self):
        ...
        if self.inventory.shovel:
            room_items["pelle"] = 0
        if self.inventory.hammer:
            room_items["marteau"] = 0
        if self.inventory.lockpick_kit:
...
class Blue(Room):
    ...
    def enter_room(self) :
        ...
        if self.name == "Sauna" :
            ...
            self.inventory.steps += extra_steps
...
class Red(Room):
    ...
    def enter_room(self) :
        ...
        if self.name == "Gymnasium" :
            message.append(f"Après un long entraînement, vous êtes épuisés ! Vous perdez {self.inventory.steps - 2} pas. Reposez-vous, avant de continuer.")
            self.inventory.steps -= 2
